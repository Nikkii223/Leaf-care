...
if (isset($_POST["gender"])){
	...
	if (!empty($_FILES) && isset($_FILES['doc_document'])) {
		    $document_image= addslashes(file_get_contents($_FILES['doc_document']['tmp_name']));
   		if (isset($_FILES['doc_document']['name'])) {
		   // echo "hiadis	husdabasdu";
			    // echo "Whut is : <br>". $image;
    		// echo "Nooooooooo : <br>" . $image_name;

			$document_name = addslashes($_FILES['doc_document']['name']);
		}else{
	...
	if (!$errors){
		...
		$sql = "INSERT INTO registerdoctor (D_Fname , D_Lname, D_Gender, Specialization, StartingHours, EndingHours, PhoneNumber, Address, Email, D_Password, Image, ImageName,d_document,document_name,d_reg_no	 )
			VALUES ('".$_POST["first_name"]."', '".$_POST["last_name"]."', '".$_POST["gender"]."', '".$_POST["specialization"]."', '".$_POST["startingTime"]."', '".$_POST["endingTime"]."', '".$tel."', '".$_POST["address"]."', '".$_POST["email"]."', '".$password."','".$image."','".$image_name."','".$document_image."','".$document_name."','".$_POST["doc_reg_number"]."')";
